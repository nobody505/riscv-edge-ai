...
class SimpleTokenizer:
    """Pure Python BPE detokenizer — replaces ONNX decoder (no libortextensions.so needed)"""
    ...
    def decode(self, input_ids):
        ...
        for i in input_ids:
            if 0 <= i < len(self.tokens):
                t = self.tokens[i]
                if t.startswith("▁"):  # sentencepiece space marker
                    pieces.append(" " + t[1:])
                else:
                    pieces.append(t)
...
class SenseVoiceSmall:
        ...
        **kwargs,
        ...
        if model_dir is None or not Path(model_dir).is_dir():
            raise FileNotFoundError(
                "SenseVoice model directory is missing; runtime download is disabled"
            )
        ...
        if not os.path.exists(model_file):
            raise FileNotFoundError(
                "SenseVoice ONNX model is missing; runtime export is disabled"
            )

        config_file = os.path.join(model_dir, "config.yaml")
        cmvn_file = os.path.join(model_dir, "am.mvn")
        config = read_yaml(config_file)
        # Pure Python BPE tokenizer (no libortextensions.so needed)
    ...
    def __call__(self, wav_content: Union[str, np.ndarray, List[str]], **kwargs):
        ...
        if len(language_list) not in (1, waveform_nums):
            raise ValueError(
                "parsed language list must contain one item or one per waveform"
            )
        if len(textnorm_list) not in (1, waveform_nums):
            raise ValueError(
                "parsed text-normalization list must contain one item or one per waveform"
            )
